...
namespace GLTF.Schema
{
    public class Accessor : GLTFChildOfRootProperty
    {
        ...
        private static int GetByteElementAsInt(byte[] buffer, uint byteOffset)
        {
            return Convert.ToInt32(GetByteElement(buffer, byteOffset));
        }

        private static int GetUByteElementAsInt(byte[] buffer, uint byteOffset)
        {
            return Convert.ToInt32(GetUByteElement(buffer, byteOffset));
        }

        private static uint GetByteElementAsUInt(byte[] buffer, uint byteOffset)
        {
            return Convert.ToUInt32(GetUByteElement(buffer, byteOffset));
        }
        ...
        private static uint GetUByteElementAsUInt(byte[] buffer, uint byteOffset)
        {
            return (uint)buffer[byteOffset]; // should only be byte size long
        }

        private static int GetShortElementAsInt(byte[] buffer, uint byteOffset)
        {
            return Convert.ToInt32(GetShortElement(buffer, byteOffset));
        }

        private static int GetUShortElementAsInt(byte[] buffer, uint byteOffset)
        {
            return Convert.ToInt32(GetUShortElement(buffer, byteOffset));
        }

        private static uint GetShortElementAsUInt(byte[] buffer, uint byteOffset)
        {
            return Convert.ToUInt32(GetShortElement(buffer, byteOffset));
        }

        private static uint GetUShortElementAsUInt(byte[] buffer, uint byteOffset)
        {
            return Convert.ToUInt32(GetUShortElement(buffer, byteOffset));
        }

        private static int GetIntElement(byte[] buffer, uint byteOffset)
        {
            return Convert.ToInt32(GetUIntElement(buffer, byteOffset));
        }
        ...
        public int[] AsIntArray(ref NumericArray contents, byte[] bufferViewData, uint offset)
        {
            if (contents.AsInts != null)
            {
                return contents.AsInts;
            }
            ...
            var GetSignedDiscreteElement = GetSignedFuncForType(ComponentType);

            for (uint idx = 0; idx < Count; idx++)
            {
                if (ComponentType == GLTFComponentType.Float)
                    arr[idx] = (int)System.Math.Floor(GetFloatElement(bufferViewData, totalByteOffset + idx * stride));
                else
                    arr[idx] = GetSignedDiscreteElement(bufferViewData, totalByteOffset + idx * stride);
            }
        ...
        private static Func<byte[], uint, int> GetSignedFuncForType(GLTFComponentType type)
        {
            switch (type)
            {
                case GLTFComponentType.Byte:
                    {
                        return GetByteElementAsInt;
                    }
                case GLTFComponentType.UnsignedByte:
                    {
                        return GetUByteElementAsInt;
                    }
                case GLTFComponentType.Short:
                    {
                        return GetShortElementAsInt;
                    }
                case GLTFComponentType.UnsignedShort:
                    {
                        return GetUShortElementAsInt;
                    }
                case GLTFComponentType.UnsignedInt:
                    {
