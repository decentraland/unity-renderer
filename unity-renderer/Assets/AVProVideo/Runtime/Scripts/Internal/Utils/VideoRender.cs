...
//			if (material.HasProperty(VideoRender.PropTextureMatrix.Id))
		...
		public static void SetupResolveMaterial(Material material, VideoResolveOptions options, bool usingAndroidOES)
		{
		...
		public static RenderTexture ResolveVideoToRenderTexture(Material resolveMaterial, RenderTexture targetTexture, ITextureProducer texture, ResolveFlags flags, ScaleMode scaleMode = ScaleMode.StretchToFill)
		{
			...
			GetResolveTextureSize(texture.GetTextureAlphaPacking(), texture.GetTextureStereoPacking(), eyeMode, texture.GetTexturePixelAspectRatio(), ref targetWidth, ref targetHeight);
