...
namespace DCL.LoadingScreen.Test
{
    public class LoadingScreenControllerTests
    {
        ...
        public void SetUp()
        {
            ...
            shaderPrewarm.PrewarmAsync(Arg.Any<Action<ShaderVariantCollection, float>>(), Arg.Any<CancellationToken>()).Returns(UniTask.CompletedTask);
        ...
        [Category("EditModeCI")]
        [Test]
        public async Task HandleRandomPositionCorrectly()
        {
            //Arrange
            int simulatedTimeout = 100;
            loadingScreenController.timeoutController.currentEvaluatedTimeout = simulatedTimeout;
        ...
        [Category("EditModeCI")]
        [Test]
        public void ShaderAreBeingPrewarmed()
        {
            //Initial signup flow
            commonDataStore.isSignUpFlow.Set(true);

            //Assert
            shaderPrewarm.Received().PrewarmAsync(Arg.Any<Action<ShaderVariantCollection, float>>(), Arg.Any<CancellationToken>());
        }
    }
}
