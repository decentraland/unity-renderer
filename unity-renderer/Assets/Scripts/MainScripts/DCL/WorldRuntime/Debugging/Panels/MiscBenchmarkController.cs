...
namespace DCL
{
    ...
    public class MiscBenchmarkController : MonoBehaviour, IBenchmarkController
    {
        ...
        void Update()
        {
            MessagingControllersManager messagingManager =
                Environment.i.messaging.manager as MessagingControllersManager;
        ...
        private IEnumerator RefreshProfilingData()
        {
            while (true)
            {
                ...
                busesLog +=
                    $"{MessagingBusType.UI.ToString()} bus: {pendingMessagesCount[MessagingBusType.UI.ToString()]} replaced: {messagesReplaced[MessagingBusType.UI.ToString()]}\n";
                busesLog +=
                    $"{MessagingBusType.INIT.ToString()} bus: {pendingMessagesCount[MessagingBusType.INIT.ToString()]} replaced: {messagesReplaced[MessagingBusType.INIT.ToString()]}\n";
                busesLog +=
