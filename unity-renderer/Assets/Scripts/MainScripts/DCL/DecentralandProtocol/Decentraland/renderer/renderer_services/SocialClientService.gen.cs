...
public interface IFriendshipsService<Context>
{
  ...
  UniTask<UpdateFriendshipResponse> UpdateFriendship(UpdateFriendshipPayload request, Context context, CancellationToken ct);
...
public static class FriendshipsServiceCodeGen
{
  ...
  public static void RegisterService<Context>(RpcServerPort<Context> port, IFriendshipsService<Context> service)
  {
    ...
    result.definition.Add("UpdateFriendship", async (payload, context, ct) => { var res = await service.UpdateFriendship(UpdateFriendshipPayload.Parser.ParseFrom(payload), context, ct); return res?.ToByteString(); });
