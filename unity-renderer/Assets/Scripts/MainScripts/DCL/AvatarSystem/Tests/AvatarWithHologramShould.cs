    ...
    public class AvatarWithHologramShould
    {
        ...
        public IEnumerator ThrowIfLoaderFails() =>
            UniTask.ToCoroutine(async () =>
            {
                ...
                curator.Configure()
                       .Curate(Arg.Any<AvatarSettings>(), Arg.Any<IEnumerable<string>>(), Arg.Any<IEnumerable<string>>(), Arg.Any<CancellationToken>())
                       .Returns(x => new UniTask<(BodyWearables bodyWearables, List<WearableItem> wearables, List<WearableItem> emotes)>((bodyWearables, wearables, emotes)));
                ...
                loader.Received().Load(bodyWearables, wearables, settings, Arg.Any<SkinnedMeshRenderer>(), Arg.Any<CancellationToken>());
            });
