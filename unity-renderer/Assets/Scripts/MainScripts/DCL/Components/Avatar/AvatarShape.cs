    ...
    public class AvatarShape : BaseComponent, IHideAvatarAreaHandler, IHidePassportAreaHandler
    {
        ...
        public override IEnumerator ApplyChanges(BaseModel newModel)
        {
            ...
            if (avatar.status != IAvatar.Status.Loaded || needsLoading)
            {
                ...
                // NOTE (Kinerius): Since users can spawn avatar shapes with zero scale,
                // we avoid the glitches generated by it by moving this object elsewhere with a normal scale (#2997)
                var thisTransform = armatureContainer.transform;
