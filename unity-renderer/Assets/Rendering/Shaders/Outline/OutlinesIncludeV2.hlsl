...
void DepthSobel_float(float2 UV, float Thickness, out float Out) 
{
    ...
    {
        float depth = SampleSceneDepth(UV + sobelSamplePoints[i] * Thickness).r;  //SHADERGRAPH_SAMPLE_SCENE_DEPTH(UV + sobelSamplePoints[i] * Thickness);
...
void GetSceneDepthRaw(float2 UV, out float Out)
{
    Out = SampleSceneDepth(UV.xy);
}

void GetSceneDepthEye(float2 UV, out float Out)
{
    Out = LinearEyeDepth(SampleSceneDepth(UV.xy), _ZBufferParams);
}
#endif