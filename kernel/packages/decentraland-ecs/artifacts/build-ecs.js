...
function watchFile(fileName, services, files) {
...
function emitFile(fileName, services) {
    ...
    output.outputFiles.forEach(o => {
        ...
        let ret = `/*! decentraland-ecs@${ecsVersion} */;\n${ecsPackageECS};\n` +
            `/*! dcl-amd */;\n${ecsPackageAMD};\n` +
            `/*! code */;\n${generatedCode}`;
        ...
        ensureDirectoriesExist(path_1.dirname(o.name));
...
function logErrors(fileName, services) {
...
function getConfiguration(sceneJson) {
    const host = {
    ...
    if (hasError) {
        ...
        console.log(util_1.inspect(tsconfig.options, false, 10, true));
        process.exit(1);
    }
    if (PRODUCTION) {
...
function printDiagnostic(diagnostic) {
    ...
    if (diagnostic.file) {
        ...
        console.log(`  Error ${diagnostic.file.fileName.replace(ts.sys.getCurrentDirectory(), '')} (${line + 1},${character +
            1}): ${message}`);
    }
    else {
...
function ensureDirectoriesExist(folder) {
    if (!ts.sys.directoryExists(folder)) {
        ensureDirectoriesExist(path_1.resolve(folder, '..'));
