...
glob
    ...
    const mappingData = {
        publisher: (manifest._blockhainMock && manifest._blockhainMock.operator) ||
    ...
    for (let p in manifest.scene.parcels) {
        ...
        ipfsMock.push(Object.assign(Object.assign({}, mappingData), { parcel_id: parcelPosition, parcelPosition }));
    }
});
...
Promise.all(parcelInfos.map(info => {
    return new Promise((resolve, reject) => fs.writeFile(path.join(parcelInfoDir, info.rootCid), JSON.stringify(info.mappingData, null, 2), err => err ? reject(err) : resolve()));
})).catch(catchAll);
Promise.all(sceneMappings.map(mapping => {
    return new Promise((resolve, reject) => fs.writeFile(path.join(sceneMappingDir, mapping.parcelPosition), JSON.stringify(mapping), err => err ? reject(err) : resolve()));
})).catch(catchAll);
function hash(str) {
...
function generateIPFSMock(folder) {
    const mappings = [];
    const files = new Set(glob.sync(folder + '/**/*', { absolute: true }));
    files.forEach(file => {
        try {
