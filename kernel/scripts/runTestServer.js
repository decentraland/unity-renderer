...
const broker_1 = require("../packages/shared/comms/proto/broker");
const titere = require("titere");
const WebSocket = require("ws");
const http = require("http");
const proto = require("../packages/shared/comms/proto/broker");
...
wss.on('connection', function connection(ws, req) {
    ...
    ws.on('message', message => {
        ...
        if (msgType === proto.MessageType.PING) {
            ...
            topicFwMessage.setRole(broker_1.Role.CLIENT);
            topicFwMessage.setBody(topicMessage.getBody_asU8());
            const topicData = topicFwMessage.serializeBinary();
            // Reliable/unreliable data
            connections.forEach($ => {
...
function checkDiff(imageAPath, imageBPath, threshold, diffOutputPath) {
...
/// --- SIDE EFFECTS ---
{
    ...
    app.use('/@/artifacts/preview.js', express.static(path_1.resolve(__dirname, '../static/dist/preview.js'), {
        setHeaders: res => {
            res.setHeader('Content-Type', 'application/javascript');
        }
    }));
    app.use('/@', express.static(path_1.resolve(__dirname, '../packages/decentraland-ecs')));
    app.use('/preview.html', express.static(path_1.resolve(__dirname, '../static/preview.html'), {
        setHeaders: res => {
            res.setHeader('Content-Type', 'text/html');
        }
    }));
    function getAllParcelIdsBetween(coords) {
        const result = [];
        try {
    ...
    function readAllJsonFiles(filenames) {
        return Promise.all(filenames.map(value => {
    ...
    const sceneEndpoint = async (req, res) => {
        ...
        const fileData = await readAllJsonFiles(coords.map(coord => path_1.resolve(__dirname, path.join('..', 'public', 'local-ipfs', 'scene_mapping', coord))));
    ...
    const parcelInfoEndpoint = async (req, res) => {
        ...
        const fileData = await readAllJsonFiles(cids.map(_ => path_1.resolve(__dirname, path.join('..', 'public', 'local-ipfs', 'parcel_info', _))));
        return res.json({
            data: fileData
                .filter($ => !!$)
                .map(($) => ({
