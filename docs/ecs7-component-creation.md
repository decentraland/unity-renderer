...
After creating the PR, we can test the @dcl/protocol inside the PR. Going to the `ProtobufEditor.cs` and change the following line of code:
```
...
```
You should change the link with the generated package in your PR. A GitHub Bot will comment on your PR with the generated link, and with every push, it will update it. (this must be temporal for testing the PR's)

After merging @dcl/protocol PR, we can let that URL empty to use the official package (@dcl/protocol@next).

After setting the location to consume the @dcl/protocol, we need to generate the code in the renderer to use the model.
For that, we have an auto-generator of the code that automatically will get the last version and generate all the code. However, to update the code, we must call this function manually from the unity renderer.

To generate the code, you must open the unity-renderer project. Then at the top of unity ( with the File, Edit, Assets, etc.), you will find another one called `Decentraland`. Then you must click on `Decentraland > Protobuf > UpdateModels`
