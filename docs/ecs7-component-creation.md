...
> **_NOTE:_**  After creating the PR, a GitHub Bot will create a comment with the link of the package to test the PR. You can use that link for testing in the following steps
...
- Yo must add the following code to enumerate the component
```
...
After creating the PR, we can test the @dcl/protocol inside the PR. Going to the `ProtobufEditor.cs` and changing the following line of code:
```
...
```
You should change the link with the generated package in your PR. A GitHub Bot will comment in your PR with the generated link, and every push it will update it. (this must be temporal for testing the PR's)

After the @dcl/protocol PR is merged, we can let that URL empty to use the official package (@dcl/protocol@next).
