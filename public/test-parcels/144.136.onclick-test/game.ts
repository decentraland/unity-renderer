import { Entity, ISystem, engine, Vector3, Transform, OnClick, Material, BoxShape, PlaneShape, Color3, Camera } from 'decentraland-ecs/src'
...
class OnClickObject{
  ...
  constructor(pos : Vector3, startingMaterialIndex : number){
    ...
    cube.add(new Transform({position: pos}));
    cube.add(new BoxShape());
    cube.set(materials[startingMaterialIndex]);

    const plane = new Entity();
    this.planeTransform = new Transform({position: pos.add(new Vector3(0,1,0)), scale: new Vector3(0.25,0.25,1)});
    plane.add(new PlaneShape());
    plane.add(this.planeTransform);
    plane.set(materials[GetNextMaterialIndex(startingMaterialIndex)]);
    ...
    cube.add(new OnClick(()=>{
      this.materialIndex = GetNextMaterialIndex(this.materialIndex);
      cube.remove(Material);
      plane.remove(Material);
      cube.set(materials[this.materialIndex]);
      plane.set(materials[GetNextMaterialIndex(this.materialIndex)]);
    }));
  }
