...
public class GLTFLoadingTestController : MonoBehaviour
{
  ...
  void OnEnable()
  {
    ...
    {
      gltfRenderers[i].OnModelFinishedLoading += SendLoadingTimeDataToEndpoint;
  ...
  void SendLoadingTimeDataToEndpoint(float loadingTime)
  {
    ...
    //data.events[0].timestamp = (Mathf.Floor((DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1, 0, 0, 0)).Milliseconds) / 1000)).ToString();

    string parsedJSON = JsonUtility.ToJson(data);

    /*Debug.Log("resulting JSON:");
    Debug.Log(paredJSON);
    yield return true;*/
