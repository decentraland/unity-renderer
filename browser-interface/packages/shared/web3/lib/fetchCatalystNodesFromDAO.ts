...
import { catalystABI } from 'shared/web3/lib/catalystABI'
import { getNetworkFromProvider } from './provider'

/**
 * HARDCODED_CATALYST_LIST exists because this function takes too much to fetch all the data from the contract
 */
export async function fetchCatalystNodesFromContract(): Promise<CatalystNode[]> {
  ...
  const net = await getNetworkFromProvider()
