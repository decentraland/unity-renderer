...
export function hookConnectToFixedAdaptersIfNecessary(realm: IRealmAdapter) {
  ...
  if (realm.about.comms?.adapter.startsWith('fixed')) {
    const adapter = realm.about.comms?.adapter
    const ix = adapter.indexOf(':')
    connStr = adapter.substring(ix + 1)
  } else if (realm.about.comms && 'fixedAdapter' in realm.about.comms) {
    trackEvent('DEFAULT_REALM', {
      message: `fixed adapter: ${JSON.stringify(realm.about.comms?.fixedAdapter as string)}`
    })
