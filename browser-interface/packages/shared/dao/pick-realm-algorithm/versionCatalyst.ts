...
const serviceVersionChecker = {
  content: {
    ...
    isMet: (requiredVersion: string, candidate: Candidate) => !semver.valid(candidate.version.content) || semver.gte(candidate.version.content, requiredVersion)
  },
  lambdas: {
    isValid: semver.valid,
    isMet: (requiredVersion: string, candidate: Candidate) => !semver.valid(candidate.version.lambdas) || semver.gte(candidate.version.lambdas, requiredVersion)
  },
  bff: {
    isValid: semver.valid,
    isMet: (requiredVersion: string, candidate: Candidate) => !semver.valid(candidate.version.bff) || semver.gte(candidate.version.bff, requiredVersion)
  },
  comms: {
...
export function versionCatalystLink(minimumVersions?: VersionCatalystParameters): AlgorithmLink {
  return {
    ...
    pick: (context: AlgorithmContext) => {
      ...
      minimumVersions && Object.entries(minimumVersions).forEach(([serviceName, requiredVersion]) => {
